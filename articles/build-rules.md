...
Executes an R script using `Rscript`. The rule is triggered when any
...
- `params` - List of R values that become available within the script in
  a `params` variable
...
- `depends` - Vector of file names that the markdown script depends on,
  or `NULL`
- `format` - Output format specification (see below)
- `params` - List of R values available within the script in a `params`
  variable
- `task` - Character vector of parent task names
...
[`knitr::knit()`](https://rdrr.io/pkg/knitr/man/knit.html). This is
...
- `depends` - Vector of file names that the knitr script depends on, or
  `NULL`
- `params` - List of R values available within the script in a `params`
  variable
- `task` - Character vector of parent task names
...
- `depends` - Name of the file to copy from (only the first element is
  used)
- `task` - Character vector of parent task names
...
Defines a dependency between targets without providing any execution
script. This is useful when you want to specify that a target depends on
...
Runs the make process in a subdirectory. The subdirectory is assumed to
contain its own `Makefile`. This rule executes `make <targetTask>` in
...
- `task` - Character vector of parent task names
- `targetTask` - What task to execute in the subdirectory (default:
  “all”)
...
message instructing the user to perform a task manually. This is useful
