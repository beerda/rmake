...
%% almost as usual
\author{Michal Burda\\University of Ostrava}
...
\Abstract{
  \R{} is a scripting language for statistical analyses that are \emph{repeatable}, i.e. easily
  re-computed if data or some data processing step changes. However, if the analyses grow on complexity,
  their manual re-execution on any change may become tedious and prone to errors. \emph{Make} is a tool
  for managing the generation of resulting files from source data and script files. \emph{Make} reads
...
\Keywords{build process, \code{make}, \code{Makefile}, \R{}}
\Plainkeywords{build process, make, Makefile, R} %% without formatting
...
re-execute the whole process after changes in data or pre-processing steps.

There are several useful packages for \R{} to make repeatability of statistical computations much more easier.
Among others, let us name e.g. \pkg{knitr} \citep{knitr2, knitr}
...
Creation of statistical reports is as simple as issuing a single statement from the command line or as clicking an
icon in an integrated development environment (IDE) such as \emph{RStudio} \citep{rstudio}.

However, if the analyses grow on complexity, manual re-execution of the whole process may become tedious and prone
to errors as well as very demanding from the view of computational power.
Complex analyses involve typically a lot of pre-processing steps on large data sets, a repetitive execution of commands
with differing parameters, and producing multiple output files in
...
and save intermediate results into a file. However, such approach brings another challenge: management of dependencies between
...
script files. It was primarily created to help program developers to build binaries from source codes. However, it can be
used easily to run generation of any types of files. \emph{Make} gets its knowledge of how to build the results from a file
called the \code{Makefile}, which defines dependencies between files and commands of how to create dependent files
...
\emph{Make} is also quite popular in the \R{} community, with a direct support e.g. by \emph{RStudio} and other tools.
...
\item easy definitions of file dependencies in \R{} language;
\item high flexibility provided by parameterized execution of \R{} scripts and programmatically generated dependencies;
\item simple and short coding thanks to special \verb|%>>%| pipeline operator and templating mechanism;
\item support for rules based on execution of \R{} scripts and \R{} markdown files;
...
\todo{other makefile generators:}
https://github.com/GShotwell/easyMake
...
between \R{} objects by itself. To the opposite, \pkg{rmake} is simply a generator of the \code{Makefile} dependency file,
which leaves the re-generation of obsolete results on the \emph{Make} utility. A large list of other pipelining projects
may be found at \code{https://github.com/pditommaso/awesome-pipeline}.

The rest of the paper is organized as follows. Section~\ref{sec:installation} lists all necessary steps for correct
installation and setup of the environment. Section~\ref{sec:basic} describes the basic usage of the \pkg{rmake} package
including new project initialization, creating the build rules, running the build process, and visualization of
the file dependencies. Section~\ref{sec:details} provides all details on pre-defined build rules and custom rules
definition. In Section~\ref{sec:advanced}, the advanced topics of \pkg{rmake} usage are discussed: the mechanism
...
On Linux-based systems, it is usually a matter of installing the appropriate distribution packages. On Windows, an installation
of \code{Rtools} is recommended that contains the \emph{Make} interpreter included.

To install the \pkg{rmake} package from CRAN, execute the following package from within the \R{}
session (note that leading ``\texttt{>}'' denotes the \R{}'s shell prompt and it is not a part of the command):
<<eval=FALSE>>=
...
To set environment variables on Unix-like system, issue the following shell command (``\texttt{\$}'' is a shell prompt):
\begin{verbatim}
$ export R_HOME=/usr/lib/R
$ export R_ARCH=
\end{verbatim}
These commands may be added to your home \code{.profile} file for variables to be created automatically after your log-in.
...
generate \code{Makefile}. That script file may be created manually of from skeleton provided by \pkg{rmake}. To start
from skeleton, first load the \pkg{rmake} package
<<>>=
library(rmake)
@
...
This will create two files in the current directory (``\texttt{.}''): \code{Makefile.R} and \code{Makefile}.
...
an automatic re-creation of \code{Makefile} after any change to the \code{Makefile.R} script.


\subsection{Running the Build Process}

Once \code{Makefile} exists, the \emph{Make} tool may be executed from within the \R{} session by calling the following function:
<<eval=FALSE>>=
make()
@
\begin{verbatim}
...
in \emph{Build}/\emph{Configure Build Tools}, set \emph{Project build tools} to \emph{Makefile}. A \emph{Build All}
...
whose target is  \code{sums.csv} and which depends on \code{data.csv}. Whenever any dependency or the script file
changes, the rule triggers and re-executes the given script in order to build the given target. Let us
run the \code{make} command from the shell again:
<<eval=FALSE>>=
make()
@
Make should firstly re-generate the \code{Makefile} itself (since \code{Makefile.R} has changed) and then
execute \code{script.R} in a new \R{} session to create \code{sums.csv}. Further calls of \code{make}
...
\begin{figure}
  ...
  \begin{tikzpicture}[auto]
    ...
    \path [line] (rmd)    -- (pdf);
  \end{tikzpicture}
  \caption{Simple chain of dependencies from the example in Section~\ref{sec:addrule}}
...
a prerequisite for \code{analysis.pdf}. Such sequence of rules may be equivalently written using the special
``\pipe{}'' pipe operator (cf. with Fig.~\ref{fig:basicUsageChain}):
<<>>=
...
a call to a function that creates a \pkg{rmake} rule. Prior its execution, the function call internally obtains two additional named arguments: \code{depends} and \code{target}, whose values are respectively obtained from the preceding (i.e., $(k-1)$-th) or following (i.e., $(k+1)$-th) element of the chain.

If the dependencies are more complex than a single chain, multiple chains may be specified and merged with the
\code{c()} function:
<<>>=
...
$ make clean
\end{verbatim}
Each \pkg{rmake}'s rule type adds to the \code{Makefile} a command for deleting all target files that were generated by that rule.
...
The list of rules may be printed, to see concisely the defined dependencies. For instance, the \texttt{job} from Section~\ref{sec:pipes} would produce the following output:
<<>>=
print(job)
@

However, a lot more comprehensible view on the graph of dependencies is obtained by visualizing the \texttt{job}:
<<eval=FALSE>>=
...
\begin{figure}
    ...
    \caption{Visualization of the \texttt{job} from Section~\ref{sec:pipes} with the \texttt{visualize()} function}
    \label{fig:pipefig}
\end{figure}

The \texttt{visualize()} function converts the \texttt{job} into a \pkg{visNetwork}'s directed graph and renders it as an interactive web-page, in which the picture may be zoomed and nodes re-arranged with a mouse -- see Fig.~\ref{fig:pipefig} for an overview. The optional \texttt{legend} argument turns off the legend in the resulting figure.
...
The \pkg{rmake} package provides several functions to create the most common rule types.
Each function has a mandatory \code{target} argument for a character vector of names of files
to be created by tha rule. Additionally, each function has an optional \code{depends} argument, which is a character vector of file names the rule depends on. The \code{task}
argument allows rules to group into \emph{tasks} -- see Section~\ref{sec:tasks} for more details. Some rules also allow an optional \code{params} argument that allows to pass parameters to the scripts; Section~\ref{sec:params} contains detailed information on that topic. Each rule, when executed by the \emph{Make} tool, is started as a separate operating system process, that is, the \R{} scripts and markdown processors do not share the running instance of the \R{} interpreter.
...
The \code{rRule()} rule executes the \R{} script by calling the \code{Rscript} binary from the shell and \code{source}-ing the \code{script} file name. A rule is fired if either any file from \code{depends} or the \code{script} file changes.
...
Besides predefined rule types, any custom rule may be added to the build process. The \code{makefile()} function generates \code{Makefile} from a list of instances of S3 class \code{rmake.rule}. To create an object of that type, the following function may be used:
<<eval=FALSE,prompt=FALSE>>=
rule(target, depends = NULL, build = NULL, clean = NULL,
     task = "all", phony = FALSE)
@

\code{target} is a character vector of target files that the rules is intended to generate. \code{depends} is a character vector of prerequisite file names. To the contrast with predefined rule types, custom rules should add also script file names into a vector of dependencies, preceding other data files.

The \code{task} argument may be used to assign the rule to a certain task, see Section~\ref{sec:tasks} for more information.

The \code{phony} argument is a boolean (\code{TRUE}/\code{FALSE}) specifying whether the \emph{Make} rule has a \code{.PHONY} (i.e. non-file) target. A rule should be marked with \code{phony} if the target is not a file name that would be generated by the \code{build} command. E.g., \code{all} or \code{clean} are phony targets. Also all targets representing tasks (see Section~\ref{sec:tasks}) are phony. See also the manual to the \emph{Make} tool for more details on \code{.PHONY} targets.
...
\emph{Make} variables other than \code{\$(R)} or \code{\$(RM)} may be defined by modifying the \code{defaultVars} global variable, e.g., introduce a new \code{\$(JS)} variable with path to the \proglang{JavaScript} interpreter:
<<eval=FALSE,prompt=FALSE>>=
...
The \pkg{rmake} package provides a useful function to help write rules that execute custom \R{} sequence of commands. The \code{inShell()} function simply takes an \R{} language expression and transforms it into a character vector with a shell command that calls \code{Rscript} and runs that expression:
<<>>=
inShell({ result <- 1+1; saveRDS(result, 'result.rds') })
@
That code obtains a result of a simple mathematical operation and stores the result into \code{result.rds}. The \code{inShell()} function may then be used to define a new rule:
<<eval=FALSE,prompt=FALSE>>=
rule(target='result.rds',
     build=inShell({ result <- 1+1; saveRDS(result, 'result.rds') }),
     clean='$(RM) result.rds')
@
However, overuse of \code{inShell()} is not recommended. For normal processing of data, it is far better to store \R{} commands into a separate script file, because then, after any change to the code is made, the \emph{Make} tool can detect it and force update of other depending artifacts. Any rule change at the level of \code{Makefile} will not cause any rebuild of the target. The intended use of \code{inShell()} is to ease the implementation of internals such as in \code{rRule()} or \code{markdownRule()}.
...
A rule is assigned to a task by specifying task name in the argument of rule creation function. A rule may be a member of more than one task -- simply put all task names in the character vector during the initialization of a rule.

In the following example, several rules are assigned to two tasks:
<<eval=FALSE,prompt=FALSE>>=
library(rmake)
job <- c(
...
makefile(job, "Makefile")
@
This creates a rule \code{preprocess.R}, which transforms \code{data.csv} into \code{data.rds}, and two markdown rules, \code{preview.Rmd} and \code{final.Rmd}, that are each assigned to its own task named \code{preview} and \code{final}, respectively. Thus invoking
<<eval=FALSE>>=
make('preview')
@
would create \code{data.rds} (because \code{preview.Rmd} depends on it, irrespective of \code{preprocess.R} is a member of \code{all} task) and \code{preview.pdf}, but not \code{final.pdf}. Similarly,
<<eval=FALSE>>=
make('final')
@
...
The \pkg{rmake} package allows to send parameters to the objective of a rule. Both \code{rRule()} and \code{markdownRule()} functions
may obtain a list of arbitrary data as the \code{params} argument. The content of the list would be available as the \code{params} global variable from within the script, once it is executed. Through such parameterization, a single \R{} script may be used to produce multiple outputs. An example follows:
...
Indeed, the \code{params} variable contains the \code{alpha} parameter with an expected value. Besides that, \code{params} contains
dot-named values that correspond to the settings of the rule: \code{.target}, \code{.script}, \code{.depends}, and \code{.task}.

The \code{fit.R} script may work with the \code{params} global variable directly, but it is advisable to use the \code{getParam()} function instead, which throws an error in case the script is executed without \code{params} being defined before:
<<eval=FALSE,prompt=FALSE>>=
# the fit.R file
library(rmake)
...
@
The warning does not disappear, but the script has now a chance to run with proper parameters, which may be useful during development and script testing in \emph{RStudio}:
<<eval=FALSE>>=
source('fit.R')
@
\begin{verbatim}
...
More complex analyzes may contain similar rule sequences that repeate multiple times. Think of fitting multiple models differing only in some parameters, stored into files with a similar name. The \pkg{rmake} package provides a templating mechanism to avoid tedious copy\&paste of rule definitions and help quickly creating and easily maintaining the \code{Makefile} generator script.
...
job <- expandTemplate(tmpl, variants)
@
The resulting \texttt{job} contains six rules that combine the specified variants as follows:
<<>>=
print(job)
@
...
@
However, the repeated rules are automatically removed:
<<>>=
print(job)
@

On the other hand, (not only) a template expansion may often result in distinct rules producing a duplicated target. Such sequence of rules is prohibited and causes an error message in the \texttt{makefile()} function. The problem is illustrated in the example below.
<<error=TRUE>>=
tmpl <- 'data-$[TYPE].csv' %>>% markdownRule('report.Rmd') %>>% 'report.pdf'
