...
<<echo=FALSE>>=
  ...
  options(prompt='R> ')
  options(continue='+  ')
  opts_chunk$set(prompt=TRUE, echo=TRUE)
@
...
However, if the analyses grow on complexity, the re-execution of the whole process may become demanding a lot of computational
power. Complex analyses involve typically a lot of pre-processing steps on large data sets and produce multiple outputs in
various formats. may be tedious and prone to errors. It is very inefficient to re-run over and over again all the pre-processing
steps to refresh the final report being developed. A caching mechanism provided by \pkg{knitr} is very helpful there,
...
which files (and in what order) have to be refreshed in order to update all of them.

\emph{Make} is also quite popular in the \R{} community, with a direct support e.g. by \emph{RStudio} and other tools.
It is quite straightforward to write \code{Makefile} manually.  However, for more complex build processes,
manual management of \code{Makefile} may become grueling. The aim of this paper is to present \pkg{rmake},
an \R{} package providing tools for easy generation of \code{Makefile}s. The main features of \pkg{rmake} are as follows:
\begin{itemize}
\item easy definitions of file dependencies and building commands from within \R{};
\item high flexibility provided by parameterized execution of \R{} scripts and programmatically generated dependencies between files;
\item support for rules based on execution of \R{} scripts, \R{} markdown files etc.;
\item parallel execution of building tasks obtained for free thanks to \emph{Make}'s parallel processing features;
\item support for all platforms including Unix (Linux), MacOS, and MS Windows;
...
The rest of the paper is organized as follows. \todo{...}
...
One can then modify the above-listed rule to use the new \code{\$(JS)} variable:
\begin{verbatim}
library(rmake)
defaultVars['JS'] <- '/usr/bin/node'
...
# now we can use these variables to do the actual work...
