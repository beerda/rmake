...
\Abstract{
  \R{} software allows the development of \emph{repeatable} statistical analyses, i.e., automatically re-computable analyses
  if data or some data processing step changes. However, if the analyses grow in complexity,
...
other benefits, an important advantage of \R{} is that it allows writing \emph{repeatable}
statistical analyses, that is, programming all steps of data processing in a scripting language, which allows
re-executing the whole process after any change in data or in any processing step.

There are several useful packages for \R{} to obtain repeatability of statistical computations much more easily.
Among others, let us mention \pkg{knitr} \citep{knitr2, knitr}
and \pkg{rmarkdown} \citep{rmarkdown}. These tools
allow writing R scripts that generate reports combining text with tables and figures generated from data.
...
However, if the analyses grow in complexity, manual re-execution of the whole process may become tedious, prone
to errors, and very demanding from the perspective of computational power.
Complex analyses typically involve a lot of pre-processing steps on large data sets, repetitive execution of commands
differing only in several parameters, and producing multiple output files in
various formats. It is very inefficient to re-run all the pre-processing
steps over and over again to refresh the final report after any change to it. A caching mechanism provided by \pkg{knitr} is very helpful there,
but its use is still limited to a single report. It is rational to split a complex analysis into several parts
and save intermediate results into files. However, such an approach brings another challenge: management of dependencies between
inputs, outputs and underlying scripts to ensure a refresh of all the results on any change to any prerequisite.
In open source community, (GNU) \emph{Make} is a popular tool to help with that.

\emph{Make} is a tool that controls the generation of intermediate and resulting files from source data and
script files. It was primarily created to help program developers build executable binaries from source codes.
However, it can be
used to generate any type of file. \emph{Make} obtains its knowledge of how to build the results from a file
...
\emph{Make} is also quite popular in the \R{} community, with direct support e.g. by \emph{RStudio} and other tools.
It is straightforward to write \code{Makefile} manually and many \R{} users write simple \code{Makefile}s for
...
A different approach to building complex analytical projects is represented e.g. by the \pkg{drake} package \citep{drake},
which (unlike \pkg{rmake}) processes all the tasks within a single \R{} session and handles all the dependencies
between \R{} objects by itself. In contrast, \pkg{rmake} is simply a light-weight generator of the \code{Makefile} dependency file,
which leaves the re-generation of obsolete results to the \emph{Make} utility. A large list of other pipelining projects
...
In order to use \pkg{rmake}, the \R{} environment and the \emph{Make} program must be installed and properly configured
in advance. On Linux-based systems, it is usually a matter of installing the appropriate distribution packages.
On Windows, installation of \code{Rtools} is recommended, which contains the \emph{Make} tool.
...
For \pkg{rmake} to work properly, \code{R_HOME} and \code{R_ARCH} environment variables must be set correctly. If executing \emph{Make} from within an \R{} session (e.g., from \emph{RStudio}), the environment variables are set automatically by \R{}. To execute \emph{Make} outside \R{}, for instance, from the shell, these variables must be set manually. The \code{R_HOME} variable should contain the path to \R{}'s installation directory and \code{R_ARCH} the architecture variant. The correct values may be obtained
...
$ export R_ARCH=
\end{verbatim}
These commands may be added to your home \code{.profile} file for the variables to be created automatically when you log in.
...
To start maintaining the \R{} project with \pkg{rmake}, an \R{} script \code{Makefile.R} must be created that will
...
does not need to be re-generated. After \code{Makefile.R} is updated, \code{Makefile} will be re-generated
and other tasks will be executed as specified by the rules in \code{Makefile.R}.
...
If working in \emph{RStudio}, it is beneficial to set up its environment to use Make:
in the \emph{Build}/\emph{Configure Build Tools} menu, set \emph{Project build tools} to \emph{Makefile}. A \emph{Build All}
command then becomes available that runs \emph{Make} using the generated \code{Makefile}.
...
@
The script reads \code{data.csv} into a variable \code{d}, creates a data frame \code{sums} with the computed
sums, and writes it to the file \code{sums.csv}.

Now, let us modify the \code{Makefile.R} script to build the \code{sums.csv}
file automatically whenever either the \code{data.csv} or \code{script.R} file changes. All that has to be done is to update
...
makefile(job, "makefile")
@
Function \code{rRule()} creates a new rule for the execution of an \R{} script \code{script.R},
whose target is \code{sums.csv} and which depends on \code{data.csv}. Whenever any dependency file or the script file
changes, the rule triggers and re-executes the given script to build the given target. Let us
run the \code{make} command again:
<<eval=FALSE>>=
make()
@
The \emph{Make} utility should first re-generate the \code{Makefile} itself (since \code{Makefile.R} has changed)
...
more details on working with markdown documents, see \cite{rmarkdown}.

The \code{analysis.Rmd} script depends on the \code{sums.csv} data file. The markdown processor produces
the \code{analysis.pdf} file from it.
Let us now update \code{Makefile.R} so that the PDF file is refreshed every time either the script or the data changes.
...
a call to a function that creates a \pkg{rmake} rule. Prior to its execution, each such function call internally obtains two additional named arguments: \code{depends} and \code{target}, whose values are respectively obtained from the preceding (i.e., $(k-1)$-th) or following (i.e., $(k+1)$-th) element of the chain.
...
A good practice for \code{Makefile} writers is to provide a clean-up task that deletes all files of the project
that were generated within the build process. This is traditionally executed by the following command:
\begin{verbatim}
$ make clean
\end{verbatim}
Each \pkg{rmake}'s rule type automatically adds to the \code{Makefile} a command for deleting all target files that were generated by that rule.
A single exception is the \code{Makefile} itself, which is never deleted although it is generated too.
...
Some implementations of the \emph{Make} utility allow building multiple targets in parallel. For instance,
\emph{GNU Make} recognizes the \code{-j} argument, which can be used to specify the number of processes to run
simultaneously. For instance,
\begin{verbatim}
$ make -j8
\end{verbatim}
causes up to 8 targets to be prepared in parallel. If the \code{-j} option is given without a number, \emph{Make}
will not limit the number of rules that can run simultaneously.
...
The \code{visualize()} function converts the \code{job} into a \pkg{visNetwork}'s directed graph and renders it as an interactive web page, in which the picture may be zoomed and nodes rearranged with a mouse -- see Fig.~\ref{fig:pipefig} for an overview. The optional \code{legend} argument turns off the legend in the resulting figure.
...
The \code{rRule()} rule executes the \R{} script by calling the \code{Rscript} executable file from the shell and \code{source}-ing the \code{script} file. This rule is triggered if either any file from \code{depends} or the \code{script} file changes.
...
The \code{offlineRule()} rule provides a way to force some non-automated action within the \emph{Make} build process. It should be used whenever transformation of the prerequisites from the \code{depends} vector into \code{target} files requires a manual action. In the case of building, a custom error \code{message} is shown that instructs the user to perform the task by hand until the \code{target} files have more recent modification timestamps than files in the \code{depends} vector. Cleaning removes all \code{target} files.


\subsection{Custom Rules}

Besides predefined rule types, any custom rule may be added to the build process. Internally, the \code{makefile()} function generates a \code{Makefile} from a list of instances of the S3 \code{rmake.rule} class. To create an object of such a type, the following general function may be used:
<<eval=FALSE,prompt=FALSE>>=
rule(target, depends = NULL, build = NULL, clean = NULL,
     task = "all", phony = FALSE)
@

where \code{target} is a character vector of target files that the rule is intended to generate, and \code{depends} is a character vector of prerequisite file names. In contrast to predefined rule types, custom rules should also add script file names to the vector of dependencies, preceding other data files.

The \code{task} argument may be used to assign the rule to a certain task, see Section~\ref{sec:tasks} for more information.

The \code{phony} argument is a boolean (\code{TRUE}/\code{FALSE}) value specifying whether the \emph{Make} rule has a \code{.PHONY} (i.e. non-file) target. A rule should be marked with \code{phony} if the target is not a file name that would be generated by the \code{build} command. E.g., \code{all} or \code{clean} are phony targets. Also, all targets representing tasks (see Section~\ref{sec:tasks}) are phony. See also the manual for the \emph{Make} tool for more details on \code{.PHONY} targets.

The \code{build} and \code{clean} arguments are character vectors with shell commands that must be executed during the build or cleaning-up, respectively. It is advisable to write shell commands using \emph{Make} variables that are predefined by \pkg{rmake} at the beginning of the \code{Makefile}:
\begin{itemize}
...
The \pkg{rmake} package provides a useful tool to help write rules that execute custom \R{} sequences of commands. The \code{inShell()} function simply takes an \R{} language expression and transforms it into a character vector with a shell command that calls \code{Rscript} with parameters that execute the given expression:
<<>>=
...
rule(target="result.rds",
     ...
     clean="$(RM) result.rds")
@
However, overuse of \code{inShell()} is not recommended. For normal processing of data, it is far better to store \R{} commands in a separate script file, because then, after any change to the code is made, the \emph{Make} tool can detect it and force an update of other depending artifacts. Any rule change at the level of \code{Makefile} will not cause any rebuild of the target. The intended use of \code{inShell()} is to ease the implementation of internals such as in \code{rRule()} or \code{markdownRule()}.
...
The mechanism of tasks allows making groups of rules. Groups of rules may then be executed together.
If not stated differently, each \pkg{rmake} rule is a member of the \code{all} task. \pkg{rmake} also provides a special \code{clean} task for project cleanup. To build all rules grouped into a task, simply invoke the \code{make} command and give it the name of the task.
For instance:
\begin{verbatim}
$ make all
\end{verbatim}
...
make("preview")
@
would create \code{data.rds} (because \code{preview.Rmd} depends on it, regardless of whether \code{preprocess.R} is a member of the \code{all} task) and \code{preview.pdf}, but not \code{final.pdf}. Similarly,
<<eval=FALSE>>=
make("final")
@
would generate \code{final.pdf} (possibly with a previous build of \code{data.rds}, if it was not done already), but not \code{preview.pdf}. All rules will be triggered by calling
<<eval=FALSE>>=
make("all")
@
...
The \pkg{rmake} package allows sending parameters to the main script of a rule. Both \code{rRule()} and \code{markdownRule()} functions
may obtain a list of arbitrary data as the \code{params} argument. The content of that argument will be available as the \code{params} global variable from within the script. Through such parameterization, a single \R{} script may be used to produce multiple outputs. An example is as follows:
...
Indeed, the \code{params} variable contains the \code{alpha} parameter with the expected value. Besides that, \code{params} contains several
...
\end{verbatim}
Meaningful default values may be assigned to the parameters via a second argument of the \code{getParam()} function:
<<eval=FALSE,prompt=FALSE>>=
dataName <- getParam(".depends", "data.csv")
resultName <- getParam(".target", "result.rds")
alpha <- getParam("alpha", 0.2)
@
The warning does not disappear, but the script now has a chance to run with proper parameters, which may be useful when debugging the script in \emph{RStudio}:
<<eval=FALSE>>=
source("fit.R")
@
\begin{verbatim}
...
More complex analyses may contain similar rule sequences that repeat multiple times. Think of fitting multiple models differing only in some parameters, stored in files with a name derived from parameter values. The \pkg{rmake} package provides a templating mechanism to avoid tedious copy-and-paste of rule definitions and to help quickly create and easily maintain the \code{Makefile.R} script.

The idea of rule templates is best presented in the following example. Let us suppose we have a lot of CSV data files that all must be processed and saved in a uniform way. We might create a script that processes all files in a loop, but that would make selective re-calculation of future changed data difficult. Instead, we could write a unique \pkg{rmake} rule for each data file:
<<eval=FALSE>>=
job <- c(
...
The \code{expandTemplate()} function simply takes a list of rules, \code{tmpl}, and replaces all occurrences of \emph{template variables} in all strings with their values provided by the \code{variants} data frame. The rules in \code{tmpl} are replicated for each row of the \code{variants} data frame.
...
On the other hand, template expansion (among other things) may often result in distinct rules producing a duplicated target. Such a sequence of rules is prohibited and causes an error message in the \code{makefile()} function. The problem is illustrated in the example below.
<<>>=
...
The presented \pkg{rmake} package provides an easy but powerful way to manage complex data manipulation processes
in \R{} using the well-known and broadly adopted \emph{Make} utility. \pkg{rmake} brings tools for the generation of the
\code{Makefile}, in which the file dependencies and build rules are defined. Advanced features of \pkg{rmake} such as
pipelining (\verb|%>>%|),
parameterized rules, or rule templates, enable quick definition of file dependencies.
