...
`rmake` is an R package providing a Makefile generator for R analytical
projects. It creates and maintains a build process for complex analytic
tasks by generating Makefiles that drive the build process through the
GNU Make tool.
...
- `man/` - Generated documentation (auto-generated, do not edit
  manually)
...
1.  **Documentation**: All exported functions must have complete
    roxygen2 documentation including:
...
2.  **Naming Conventions**:
    - Functions: camelCase for user-facing functions (e.g.,
...
1.  **Rule Structure**:
    ...
    - Rules must have: `target`, `depends`, `build`, `clean`, `task`,
      `phony` attributes
    - Use [`rule()`](https://beerda.github.io/rmake/reference/rule.md)
      function as the base constructor
2.  **Rule Functions**:
    - Pre-defined rule types:
      ...
      etc.
    - Each rule function should validate inputs and construct
...
3.  **Makefile Generation**:
    - [`makefile()`](https://beerda.github.io/rmake/reference/makefile.md)
      function generates the Makefile from a list of rules
...
3.  **Test Naming**:
    ...
    - Example:
      `test_that("rRule creates correct dependencies", { ... })`
...
- [`expandTemplate()`](https://beerda.github.io/rmake/reference/expandTemplate.md)
  function for parameterized rule generation
...
- Use
  ...
  function to access parameters safely
...
2.  [`makefile()`](https://beerda.github.io/rmake/reference/makefile.md)
    generates `Makefile` from rules
3.  [`make()`](https://beerda.github.io/rmake/reference/make.md)
    executes Make tool
...
3.  Function should:
    ...
    - Call [`rule()`](https://beerda.github.io/rmake/reference/rule.md)
...
5.  **Follow existing patterns**: Study similar functions before
    implementing new features
6.  **Platform compatibility**: Test on both Windows and Unix if making
    platform-specific changes
7.  **Maintain backward compatibility**: Changes should not break
    existing user code
...
- **R-CMD-check**: Runs on push/PR to main/master, tests on multiple OS
  and R versions
